...
public class IdeMcpAsyncServer extends McpAsyncServer {
    ...
    private McpRequestHandler<McpSchema.CallToolResult> toolsCallRequestHandler() {
        return (exchange, params) -> {
            ...
            return Mono.using(
                    () -> {
                        com.hayden.multiagentidelib.infrastructure.McpRequestContext.setHeaders(captured);
                        return Boolean.TRUE;
                    },
                    ignored -> toolSpecification.get().callHandler().apply(exchange, callToolRequest),
                    ignored -> com.hayden.multiagentidelib.infrastructure.McpRequestContext.clear()
            );
        };
    }
